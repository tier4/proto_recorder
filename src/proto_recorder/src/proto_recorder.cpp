...
: rclcpp::Node("proto_recorder", options)
{
  ...
  storage_options_.max_bagfile_size =
    static_cast<uint64_t>(declare_parameter<int64_t>("max_bagfile_size", 0));
  storage_options_.max_bagfile_duration =
    static_cast<uint64_t>(declare_parameter<int64_t>("max_bagfile_duration", 0));
  storage_options_.max_cache_size =
    static_cast<uint64_t>(declare_parameter<int64_t>("max_cache_size", 0));
  storage_options_.storage_preset_profile =
    declare_parameter<std::string>("storage_preset_profile", "");
  ...
  record_options_.compression_queue_size =
    static_cast<uint64_t>(declare_parameter<int64_t>("compression_queue_size", 1));
  record_options_.compression_threads =
    static_cast<uint64_t>(declare_parameter<int64_t>("compression_threads", 0));

  // Topic rate check parameters
  rate_check_window_size_ =
    static_cast<size_t>(declare_parameter<int64_t>("rate_check_window_size", 10));
  ...
  RCLCPP_INFO(
    this->get_logger(), "Initialized ProtoRecorder with storage_id: %s, uri: %s",
    storage_options_.storage_id.c_str(), storage_options_.uri.c_str());

  if (!storage_options_.storage_config_uri.empty()) {
    RCLCPP_INFO(
      this->get_logger(), "Using storage configuration file: %s",
      storage_options_.storage_config_uri.c_str());
  }

  if (!storage_options_.storage_preset_profile.empty()) {
    RCLCPP_INFO(
      this->get_logger(), "Using storage preset profile: %s",
...
bool ProtoRecorder::is_paused() const { return paused_.load(); }
...
void ProtoRecorder::subscribe_topics(const std::vector<std::string> & topics)
{
  ...
  for (const auto & topic : topics) {
    ...
    if (it != topics_and_types.end()) {
      ...
      RCLCPP_WARN(get_logger(), "Topic '%s' not found or not yet available.", topic.c_str());
    }
  }
}
...
{
  RCLCPP_INFO(
    get_logger(), "Subscribing to topic '%s' with type '%s'", topic_name.c_str(),
    topic_type.c_str());
  ...
  if (subscription) {
    ...
    RCLCPP_INFO(
      ...
      qos.get_rmw_qos_profile().reliability == RMW_QOS_POLICY_RELIABILITY_RELIABLE ? "RELIABLE"
                                                                                   : "BEST_EFFORT",
      qos.get_rmw_qos_profile().durability == RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL
        ? "TRANSIENT_LOCAL"
        : "VOLATILE");
  } else {
    RCLCPP_ERROR(get_logger(), "Failed to subscribe to topic '%s'", topic_name.c_str());
  }
}
    ...
    [this, topic_name, topic_type](std::shared_ptr<rclcpp::SerializedMessage> message) {
      ...
      if (is_recording_.load() && !paused_.load() && all_topics_subscribed_) {
        ...
        try {
          ...
          RCLCPP_ERROR(
            get_logger(), "Error writing message to bag for topic '%s': %s", topic_name.c_str(),
            e.what());
        }
      }
    });
...
rclcpp::QoS ProtoRecorder::adapt_qos_to_publishers(const std::string & topic_name)
{
  ...
  if (publishers_info.empty()) {
    RCLCPP_INFO_STREAM(
      this->get_logger(), "No publishers found for topic " << topic_name << ", using default QoS");
  ...
  if (reliable_publishers == num_publishers) {
    ...
    if (reliable_publishers > 0) {
      RCLCPP_WARN_STREAM(
        this->get_logger(), "Mixed reliability for publishers on topic "
                              << topic_name
                              << ". Using BEST_EFFORT to connect to all publishers, but may miss "
                                 "messages from RELIABLE publishers");
    } else {
      RCLCPP_INFO_STREAM(
        this->get_logger(),
  ...
  if (transient_local_publishers == num_publishers) {
    ...
    if (transient_local_publishers > 0) {
      RCLCPP_WARN_STREAM(
        this->get_logger(), "Mixed durability for publishers on topic "
                              << topic_name
                              << ". Using VOLATILE to connect to all publishers, but will not "
                                 "receive latched messages");
    } else {
      RCLCPP_INFO_STREAM(
        this->get_logger(),
...
void ProtoRecorder::initialize_subscriptions()
{
  if (!record_options_.topics.empty()) {
    ...
    topic_retry_timer_ = this->create_wall_timer(
      std::chrono::seconds(1), std::bind(&ProtoRecorder::retry_topics, this));
  } else {
...
void ProtoRecorder::initialize_writer()
{
  ...
  RCLCPP_INFO(
    get_logger(), "Opening writer with URI: %s, serialization format: %s",
    storage_options_.uri.c_str(), record_options_.rmw_serialization_format.c_str());

  // Create new writer instance
  writer_ = std::make_shared<rosbag2_cpp::Writer>();
  writer_->open(
    storage_options_, {rmw_get_serialization_format(), record_options_.rmw_serialization_format});
}
...
void ProtoRecorder::register_topic_to_writer(
  const std::string & topic_name, const std::string & topic_type)
{
  if (!writer_) {
    RCLCPP_WARN(
      get_logger(), "Writer is not initialized, cannot register topic '%s'", topic_name.c_str());
    return;
  }
  ...
  RCLCPP_DEBUG(
    get_logger(), "Registered topic '%s' to writer with QoS profiles", topic_name.c_str());
}
